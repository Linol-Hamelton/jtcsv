 ...
 * 
 * Usage: node benchmarks/independent-suite.ts [--scenario=all] [--iterations=5] [--output=results.json]
 */
...
Usage:
  node benchmarks/independent-suite.ts [options]
...
Examples:
  node benchmarks/independent-suite.ts --scenario=large --iterations=10
  node benchmarks/independent-suite.ts --scenario=all --output=my-results.json
    `);
    process.exit(0);
  }
});
...
const testDataGenerators = {
   ...
   */
  generateCsv(rows: number, columns = 5): string {
   ...
   */
  generateJson(rows: number, columns = 5): any[] {
    const data = [];
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < columns; j++) {
        ...
        switch (j % 4) {
          case 0: (obj as any)[key] = `value${i}_${j}`; break;
          case 1: (obj as any)[key] = i * j; break;
          case 2: (obj as any)[key] = i % 2 === 0; break;
          case 3: (obj as any)[key] = `quoted value${i}`; break;
          default: (obj as any)[key] = `data${i}_${j}`;
        }
      }
      data.push(obj);
    }
   ...
   */
  async generateLargeFile(filePath: string, rows: number): Promise<void> {
...
class Statistics {
  ...
  static confidenceInterval(values: number[], confidence = 0.95): any {
    const mean = this.mean(values);
...
class BenchmarkRunner {
  ...
  async runBenchmark(name: string, fn: () => Promise<void>, iterations = 5): Promise<any> {
    const times = [];
    const memoryStats = [];
  ...
  async runCsvToJsonBenchmark(csvData: string, libraries: any, iterations: number): Promise<any> {
  ...
  async runScenario(scenarioName: string, iterations: number): Promise<any> {
  ...
  printResults(results: any): void {
    ...
    Object.entries(results).forEach(([scenarioName, scenarioResult]) => {
      console.log(`\n${(scenarioResult as any).name.toUpperCase()}`);
      console.log('-'.repeat(40));

      // CSV to JSON results
      if ((scenarioResult as any).results['csvToJson']) {
        console.log('\nCSV → JSON Performance (mean time in ms):');
        const csvResults = Object.entries((scenarioResult as any).results['csvToJson'])
          .map(([lib, stats]) => ({
            library: lib,
            time: (stats as any).times.mean,
            memory: (stats as any).memory?.peak?.heapUsed || 0
          }))
      ...
      // JSON to CSV results
      if ((scenarioResult as any).results['jsonToCsv']) {
        console.log('\nJSON → CSV Performance (mean time in ms):');
        const jsonResults = Object.entries((scenarioResult as any).results['jsonToCsv'])
          .map(([lib, stats]) => ({
            library: lib,
            time: (stats as any).times.mean,
            memory: (stats as any).memory?.peak?.heapUsed || 0
      ...
      // Streaming results
      if ((scenarioResult as any).results['streaming']) {
        console.log('\nStreaming Performance (mean time in ms):');
        const streamResults = Object.entries((scenarioResult as any).results['streaming'])
          .map(([lib, stats]) => ({
            library: lib,
            time: (stats as any).times.mean,
            memory: (stats as any).memory?.peak?.heapUsed || 0
  ...
  saveResults(results: any, outputFile: string): void {
    const output = {
      environment,
      config: {
...
async function main() {
  ...
  for (const scenarioName of scenariosToRun) {
    ...
    try {
      ...
      allResults[scenarioName as string] = scenarioResult;
    } catch (error: any) {
