...
// Simulating Vue component for demo purposes
// In real app, this would be your .vue files

// Import the plugin
import JtcsvVuePlugin, { useJtcsv, useJtcsvAsync, csvUpload } from './index';
...
function exampleBasicConversion() {
  ...
  // Install plugin
  JtcsvVuePlugin.install(mockApp as any);
  
  // Use in Composition API style
  const jtcsv = {
    csvToJson: (csv: string, opts?: any) => {
...
function exampleAsyncOperations() {
  ...
  // Mock async jtcsv
  const jtcsvAsync = {
    csvToJsonAsync: async (csv: string, opts?: any) => {
    ...
    jsonToCsvAsync: async (data: any[], opts?: any) => {
      await new Promise(resolve => setTimeout(resolve, 10));
      if (!data.length) return '';
      const headers = Object.keys(data[0]);
      const rows = data.map(row => 
...
function exampleFileUpload() {
  ...
  handleFileUpload(mockFile as any).then(data => {
