...
class JtcsvValidator {
   ...
   */
  field(field: string, rule: any): this {
   ...
   * });
   */
  custom(name: string, validator: any): this {
   ...
   * });
   */
  row(name: string, validator: any): this {
   ...
   */
  transform(field: string, transformer: any): this {
   ...
   * }
   */
  validate(data: any, options: any = {}): any {
    const {
    ...
    if (transform && this.transformers.length > 0) {
      transformedData = transformedData.map((row, index) => {
        ...
        this.transformers.forEach(({ field, transformer }: any) => {
  ...
  filterValid(data: any): any {
    ...
    const invalidRows = new Set(errors.map((e: any) => e.row - 1));
  ...
  report(data: any): any {
    ...
    result.errors.forEach(error => {
      ...
      (errorTypes as any)[error.type] = ((errorTypes as any)[error.type] || 0) + 1;
      
      // Группировка по полю
      if (error.field) {
        (fieldErrors as any)[error.field] = ((fieldErrors as any)[error.field] || 0) + 1;
      }
    });
    ...
    return {
      ...
      analysis: {
        errorTypes,
        fieldErrors,
        mostCommonError: Object.entries(errorTypes as any).sort((a: any, b: any) => (b[1] as number) - (a[1] as number))[0],
        mostProblematicField: Object.entries(fieldErrors as any).sort((a: any, b: any) => (b[1] as number) - (a[1] as number))[0]
      },
   ...
   */
  _validateField(field: string, value: any, rule: any, row: any, rowIndex: number): any {
    const errors = [];
   ...
   */
  _checkType(value: any, type: string): boolean {
    switch (type) {
    case 'string':
   ...
   */
  _checkWarnings(row: any, rowIndex: number): any {
    const warnings = [];
   ...
   */
  _generateRecommendations(result: any): any {
    ...
    const requiredErrors = result.errors.filter((e: any) => e.type === 'REQUIRED');
    if (requiredErrors.length > 0) {
      const uniqueFields = Array.from(new Set(requiredErrors.map((e: any) => e.field)));
      recommendations.push(`Обязательные поля отсутствуют: ${uniqueFields.join(', ')}`);
    }
    
    const typeErrors = result.errors.filter((e: any) => e.type === 'TYPE');
   ...
   */
  static fromJSON(jsonSchema: any): JtcsvValidator {
    const validator = new JtcsvValidator();
    
    if (jsonSchema.fields) {
      Object.assign(validator._schema, jsonSchema.fields);
    }
    
    if (jsonSchema.rules) {
      jsonSchema.rules.forEach((rule: any) => {
        if (rule.type === 'custom') {
    ...
    if (jsonSchema.transformers) {
      jsonSchema.transformers.forEach(({ field, transformer }: any) => {
   ...
   * @returns {Object} Результат валидации
   */
  async validateCsv(csv: string, options: any = {}) {
    const { csvToJson } = require('../../../index.js');
    
    try {
      const json = await csvToJson(csv, (options as any).csvOptions || {});
      return this.validate(json, (options as any).validationOptions || {});
    } catch (error: any) {
      return {
        valid: false,
        errors: [{
  ...
  static expressMiddleware(options = {}) {
    const validator = (options as any).validator || new JtcsvValidator();
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        const result = validator.validate(data, (options as any).validationOptions || {});
        
        req.validation = result;
        
        if (!result.valid && (options as any).failOnError !== false) {
...
module.exports.schemas = (JtcsvValidator as any).schemas;
...
// Объявляем переменные для экспорта
const validateCsv = JtcsvValidator.prototype.validateCsv;
const validateJson = JtcsvValidator.prototype.validateJson;
const validateSchema = JtcsvValidator.prototype.validateSchema;
const validateFile = JtcsvValidator.prototype.validateFile;
const validateFileAsync = JtcsvValidator.prototype.validateFileAsync;
