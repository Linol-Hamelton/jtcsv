...
const LIMITS = [
  ...
  { name: 'jtcsv/errors (entry)',        file: 'dist/errors.esm.js',  limit:  3 * 1024 },
  { name: 'jtcsv (full barrel ESM)',     file: 'dist/index.esm.js',   limit: 50 * 1024 },
...
function importCost(entryFile) {
  ...
  const queue = [];
  let m;
  while ((m = re.exec(src))) queue.push(path.join(dir, m[1]));
  ...
  while (queue.length) {
    ...
    let mm;
    while ((mm = re.exec(code))) queue.push(path.join(path.dirname(next), mm[1]));
    re.lastIndex = 0;
  }
