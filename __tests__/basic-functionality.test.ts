import { jsonToCsv, preprocessData, deepUnwrap, ValidationError, LimitError } from '../index';
...
describe('Basic Functionality', () => {
  ...
  describe('deepUnwrap', () => {
    ...
    test('should respect max depth', () => {
      ...
      expect(deepUnwrap(deeplyNested, { maxDepth: 1 })).toBe('{"a":{"b":{"c":{"d":{"e":"value"}}}}}');
      // With maxDepth = 5, we should get the stringified object
      expect(deepUnwrap(deeplyNested, { maxDepth: 5 })).toBe('{"a":{"b":{"c":{"d":{"e":"value"}}}}}');
    });
  });
  ...
  describe('Input Validation', () => {
    test('should throw ValidationError for non-array input', () => {
      ...
      expect(() => jsonToCsv(null as any)).toThrow(ValidationError);
      expect(() => jsonToCsv(undefined as any)).toThrow(ValidationError);
      expect(() => jsonToCsv('string' as any)).toThrow(ValidationError);
      expect(() => jsonToCsv(123 as any)).toThrow(ValidationError);
      expect(() => jsonToCsv({} as any)).toThrow(ValidationError);
    });
    ...
    test('should respect maxRecords limit', () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain('Data size exceeds maximum limit of 10 records');
      }
    });
