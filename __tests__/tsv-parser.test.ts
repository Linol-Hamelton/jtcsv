...
describe('TsvParser', () => {
  ...
  describe('isTsv', () => {
    ...
    test('возвращает false для пустой строки', () => {
      ...
      expect(TsvParser.isTsv(null as any)).toBe(false);
      expect(TsvParser.isTsv(undefined as any)).toBe(false);
    });
  ...
  describe('validateTsv', () => {
    test('валидирует корректный TSV', () => {
      ...
      expect((result as any).stats.totalLines).toBe(3); // заголовок + 2 строки
      expect((result as any).stats.totalColumns).toBe(2);
      expect((result as any).stats.consistentColumns).toBe(true);
    });
    ...
    test('обнаруживает неконсистентные колонки', () => {
      ...
      expect((result as any).errors[0].error).toBe('Inconsistent column count');
    });
    ...
    test('возвращает ошибку для не-строки', () => {
      const result = TsvParser.validateTsv(null as any);
      expect((result as any).valid).toBe(false);
      expect((result as any).error).toBe('Input must be a non-empty string');
    });
  });
