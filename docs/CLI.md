...
```
You can also use flags instead of positional args:
```bash
jtcsv batch json-to-csv --pattern="data/*.json" --output-dir="output/"
```
`batch process` converts JSON and CSV files in the same folder. Non-CSV/JSON files are skipped.
